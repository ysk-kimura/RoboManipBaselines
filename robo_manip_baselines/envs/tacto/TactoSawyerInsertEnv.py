...
class TactoSawyerInsertEnv(TactoSawyerEnvBase):
    ...
    def setup_task_specific_object(self):
        self.obj = px.Body(
            urdf_path=path.join(
                path.dirname(__file__), "../assets/tacto/objects/cylinder/rod.urdf"
            ),
        ...
        self.obj2 = px.Body(
            urdf_path=path.join(
                path.dirname(__file__), "../assets/tacto/objects/cylinder/cylinder.urdf"
            ),
            base_position=[0.6, 0, 0.05],
            global_scaling=0.7,
            use_fixed_base=True,
        )
        self.obj3 = px.Body(
